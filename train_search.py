...
parser.add_argument('--layers', type=int, default=8, help='total number of layers') # 8
...
parser.add_argument('--resume', type=str, default='',
                    help='resume from pretrained')  # /home/cai/cai_code/Shapley-NAS/search-EXP-20230522-050112/weights_pretrain.pt
parser.add_argument('--train_portion', type=float, default=0.5, help='portion of training data')  # 训练数据比例
...
parser.add_argument('--step_size', type=float, default=0.6, help='step size for updating shapley') # 步长
parser.add_argument('--samples', type=int, default=3, help='number of samples for estimation')
parser.add_argument('--pop_size',type=int,default=25,help='') # 种群大小
parser.add_argument('--l1', type=float, default=1, help='')  # L1
parser.add_argument('--mon', type=float, default=0.4, help='')  # L2
...
def main():
    ...
    epoch = 0
    best_individual = [1e-3 * torch.randn(model.num_edges, model.num_ops).cuda(),
    ...
    for current_epochs in range(train_epochs):
        ...
        if current_epochs >= args.warm_start_epochs:
            best_normal, best_reduce = explore_architecture(valid_queue, model, criterion, epoch,
                                                            num_samples=args.samples)
            best_individual = updated_alpha(model, [best_normal, best_reduce], best_individual,
                                            momentum=args.pop_momentum, step_size=args.step_size)
...
def CFF(model, inputs, target, pop_sample, criterion, ori_pre, args):
    ...
    if ori_pre <= prec1:
        loss = args.l1 * loss + similar_loss * args.mon
    else:
        loss = args.l1 * loss - similar_loss * args.mon
...
def updated_alpha(model, shap_values, accu_shap_values, momentum=0.0, step_size=0.6):  # 步长
