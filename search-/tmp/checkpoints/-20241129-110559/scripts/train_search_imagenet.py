...
parser.add_argument('--learning_rate', type=float, default=0.5, help='init learning rate') # 初始学习率
parser.add_argument('--learning_rate_min', type=float, default=0.0, help='min learning rate')
parser.add_argument('--momentum', type=float, default=0.9, help='momentum')  # 动量
parser.add_argument('--weight_decay', type=float, default=3e-5, help='weight decay') # 权重衰减
parser.add_argument('--report_freq', type=float, default=50, help='report frequency')
parser.add_argument('--epochs', type=int, default=50, help='num of training epochs')  # 80
...
parser.add_argument('--step_size', type=float, default=0.1, help='step size for updating shapley') # 步长
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch < 15 and args.batch_size > 256:  # warm_start_epochs
...
def CFF(model,inputs,target,pop_sample,criterion,ori_pre,args):
    ...
    normal = model.module.get_projected_weights('normal').to(device)
    reduce = model.module.get_projected_weights('reduce').to(device)
    ...
    if ori_pre <= prec1:
        loss = 1*loss + similar_loss*args.mon  # L1、L2
    else:
        loss = 1*loss - similar_loss*args.mon
